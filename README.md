...
| `image_without_label` | `waterui_suspicious` (warn) | Flags a `Photo`/`Image`/`Svg` whose modifier chain carries no `.a11y_label(..)` and no `.a11y_hidden(true)` — images have no label parameter, so they enter the accessibility tree unannounced; name the image or mark it decorative, `#[allow]` covers labels set by an ancestor | No |
