...
impl<'tcx> LateLintPass<'tcx> for TypedBindingConstructor {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'tcx>) {
        ...
        let Some(ctor) = dedicated_ctor(call.value_ty) else {
            return;
        };
        let Some(arg_fix) = arg_fix(cx, call.arg, call.value_ty, ctor) else {
            return;
        };

        let mut parts: Vec<(Span, String)> = Vec::new();
        let mut applicable = Applicability::MachineApplicable;
        let ctor_text = binding_name(
            cx,
            expr.hir_id,
            expr.span.lo(),
            call.adt_did,
        ...
        span_lint_and_then(
            ...
            |diag| match ctor_text {
                Some(ctor_text) => {
                    parts.push((call.func.span, ctor_text));
                    if let Some((span, text)) = arg_fix {
                        parts.push((span, text));
                    }
                    if let Some(ann) = binding_annotation(cx, expr) {
